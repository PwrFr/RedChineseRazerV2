<template>
  <section>
    <div class="cases-row4-full container-fluid">
      <div class="row pl-5 pr-5 text-center;">
        <div class></div>
        <div
          class="col-12 cases-row4-text-range text-center"
          style="margin: 0 auto"
        >
          <div class="row cases-row4-header">
            <div class="col-12 text-center">
              MAXIMUM COOLING. MAXIMUM PERFORMANCE.
            </div>
          </div>
          <div class="col-9 text-center" style="margin: 0 auto">
            <div class="row">
              <div class="col-4 row4-col-wid-left">
                <div class="cases-row4-margin">
                  <img src="@/assets/products/3Cases/row4/1.jpg" />
                  <div class="cases-row4-des text-">VENTILATED TOP PANEL</div>
                </div>
                <div
                  class="cases-row4-des2-left text-center"
                  style="margin: 0 auto"
                >
                  Both the mini-ITX and mid-tower ATX gaming cases are designed
                  for optimized air cooling—venting heat not only from the rear,
                  but also from the top as it pulls in cool air from the front
                  and base.
                </div>
              </div>
              <!-- 2 80-->
              <div class="col-4 row4-col-wid">
                <div class="cases-row4-margin row">
                  <div class="col-12 text-center">
                    <img src="@/assets/products/3Cases/row4/2.jpg" />

                    <div class="cases-row4-des">LIQUID COOLING SUPPORT</div>
                  </div>
                </div>
                <div class="row">
                  <div class="col-12 cases-row4-des2-mid text-center">
                    Whether you’re using a custom or All-in-One (AIO) liquid
                    cooler, the Mini-ITX has enough room to support up to 240mm
                    radiators, while the ATX can fit up to 360mm to maximize the
                    cooling capabilities of your rig.
                  </div>
                </div>
              </div>
              <!-- 3 -->
              <div class="col-4 row4-col-wid-right">
                <div class="cases-row4-margin">
                  <img src="@/assets/products/3Cases/row4/3.jpg" />
                  <div class="cases-row4-des">BUILT-IN DUST FILTERS</div>
                </div>
                <div class="row">
                  <div class="col-12 cases-row4-des2-right">
                    The Razer Tomahawk ensures a clean airflow that blocks out
                    large dust particles, preventing them from reaching
                    mission-critical parts for easier maintenance and increased
                    longevity of your system.
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div class="row cases-row4-des"></div>
        </div>
      </div>
    </div>
  </section>
</template>
<style>
.row4-col-wid-left {
  max-width: 27.6875rem;
}
.row4-col-wid {
  max-width: 27.6875rem;
}
.row4-col-wid-right {
  max-width: 27.6875rem;
}
.cases-row4-full {
  padding-top: 2rem;
  background-color: #b5b5b5;
  width: 100%;
  height: 39.125rem;
  color: #000000;
  padding-left: 0rem;
}
.cases-row4-margin {
  margin-top: 0.9375rem;
  margin-bottom: -0.1875rem;
}
.cases-row4-header {
  color: #000000;
  font-family: "RazerF5", Arial, Helvetica, sans-serif;
  font-size: 1.75rem;
  font-weight: 200;
  letter-spacing: 0.0625rem;
  line-height: 1.875rem;
  margin: 1rem 0;
}
.cases-row4-des {
  font-family: "Robo-light", sans-serif;
  font-size: 1.25rem;
  font-weight: 300;
  color: #000;
  margin: 0.9375rem 0 0.625rem;
}
.cases-row4-des2-left {
  padding-left: 0.625rem;
  max-width: 21.25rem;
  font-family: "Robo-light", sans-serif;
  font-size: 1.25rem;
  color: #000;
  margin: 0.5rem 0 0.625rem;
  font-size: 1rem;
  line-height: 1.5em;
}
.cases-row4-des2-mid {
  max-width: 23.125rem;
  font-family: "Robo-light", sans-serif;
  font-size: 1.25rem;
  color: #000;
  margin: 0 auto;
  font-size: 1rem;
  line-height: 1.5em;
}
.cases-row4-des2-right {
  max-width: 23.125rem;
  font-family: "Robo-light", sans-serif;
  font-size: 1.25rem;
  color: #000;

  font-size: 1rem;
  line-height: 1.5em;
  margin: 0 auto;
}
</style>